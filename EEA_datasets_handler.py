...
def get_supported_pollutants_dict():
    ...
    Notes
    ----------
    While every pollutant has a different numeric notation, there are pollutants that have the same textual notation. (I.e.
...
def get_supported_pollutants_inverse_dict():
    ...
    Notes
    ----------
    The returned dictionary doesn't contain all the supported pollutants: are not considered the pollutants for which
    exist at least another pollutant with the same textual-notation. (I.e. duplicate text notation)
    """
...
def get_supported_countries_dict():
    ...
    Notes
    ----------
    Every country has both a different code notation and a different extended notation.
    """
...
def get_supported_countries_cities_dict():
    ...
    ----------
    dict
        Dictionary with keys the supported countries and values the associated list of cities.

    Notes
    ----------
    There are countries without supported cities: these countries has simply associated the empty list.
    """
...
def keep_pollutants_supported(pollutants):
    ...
    ----------
    list
        A new list with only supported pollutants. Each pollutant is expressed in the numeric notation.

    Warns
    ----------
    UserWarning
        When are given not supported pollutants.

    Notes
    ----------
    - `pollutants` can be the string "all": in this case the returned list contains all the supported pollutants.
    - In `pollutants` are not considered supported the pollutants expressed in textual notation for which exist at least
      another pollutant with the same textual notation. (I.e. duplicate text notation)
    """
    ...
    if len(pollutants_dif)>0:
        ...
        return [ pollutant for pollutant in pollutants if not pollutant in pollutants_dif ] # Solo pollutant supportati
...
def keep_years_supported(years):
    ...
    ----------
    list
        A new list with only supported years.

    Warns
    ----------
    UserWarning
        When are given not supported years.
    """
    ...
    if len(years_dif)>0:
        ...
        return [ year for year in years if not year in years_dif]
...
def keep_countries_cities_supported(countries_cities_dict):
    ...
    ----------
    UserWarning
        - When are given not supported countries.
        - When are given not supported cities for a certain (supported) country.
...
def download_datasets(dest_path, countries_cities_dict, pollutants, years):
    ...
    ----------
    UserWarning
        - When are given not supported countries/cities/pollutants/years.
        - When, for a specified country, isn't donwload any dataset.
        - When some problem occour during an HTTP request. (I.e. a problem during the downloading of certain datasets).

    Notes
    ---------
    In `countries_cities_dict` a country can be associated to the string "all": in this case all the supported cities of that
...
def _download_single_dataset(dest_path, country, city, pollutants, years):
    ...
    Notes
    ---------
    - There aren't done any checks on the input parameters `country`/`city`/`pollutants`/`years`: the caller of the function
    ...
    if city: # City folder
        city_supp = re.sub('[\/:*?"<>|_"]', '', city) # Tolgo i caratteri non leciti per un nome di directory
        city_path = country_path + "\\" + city_supp

    first = True # Flag which indicates if the first dataset has not been donwloaded yet

    # Iterate through all pollutants and years
...
def retrieve_datasets(source_path, countries_cities_dict, pollutants, years):
    ...
    ----------
    UserWarning
        - When are given not supported countries/pollutants/years.
        - When, for a specified country, isn't found any dataset.
    ...
    def retrieve_files(source_path, keywords): # keywords è una lista di tuple
        """
        ...
        keywords: list
            List of tuple, in which each tuple contains an indefinite number of string.
        ...
        Notes
        ----------
        A file is selected if exits at least a tuple in 'keyword' in which all the string are contained in the local path of
        the file.
        It's like that the strings inside the same tuple are in logical AND, instead different tuples are in logical OR.
        """
        # List of files local paths to return in output
        all_files = list()

        # List of files and subdirectories. The names are relative
    ...
    # For the countries withous supported cities is simply put the code country (e.g. "AD").
    ...
    for country in countries_cities_dict: # Iterate through all the specified countries
        if not supported_countries_cities_dict[country]: # Country without supported cities: put only the code country
            ...
            # Put alle the cities of that country ("country_city" for each specified city)
            # It can alse be empty list
...
def remove_datasets(source_path, countries_cities_dict, pollutants, years):
    ...
    ----------
    source_path : str
        Local path in which delete the selected datasets.
    countries_cities_dict : dict
        Map between countries and list of cities. Each country can be either expressed in the code notation or in the
        extended notation.
    pollutants : list
...
def load_datasets(source_path, countries_cities_dict, pollutants, years):
    ...
    UserWarning
        - When are given not supported countries/pollutants/years.
        - When, for a specified country, isn't found any dataset.
        - When it's not found any dataset at all.
...
def preprocessing(df, fill=True, fill_n_days=10, fill_aggr="mean"):
    ...
    fill : bool
        If True, the missing days (i.e. the days without any measurements in `df`) are filled. Otherwise they remain with a
        missing value.
        (In all the three returned DataFrame)
    fill_n_days : int or str
        The number of previous days, contained in `df`, used to fill a missing day.
        `fill_n_days` can be either an integer or the string "all": in the latter case all the previous days in `df` are used
    ...
    ----------
    pd.DataFrame
        The prepared and cleaned DataFrame, containing the daily mean concentration.
    pd.DataFrame
        The prepared and cleaned DataFrame, containing the daily min concentration.
    pd.DataFrame
        The prepared and cleaned DataFrame, containing the daily max concentration.

    Warns
    ----------
    UserWarning
        When are present missing days.
    ...
    - If `fill` is True, the missing days are filled computing the aggregation `fill_aggr` on the `fill_n_days` days in `df`
      ...
      (In all the three returned DataFrame)
      Moreover the missing days, for which are not found previous days in `df`, are deleted from the returned DataFrame.
    ...
    if fill: # Fill the missing days
        # Days to remove in the final DataFrames, because they are missing days and have not been found previous days in `df`
        days_to_remove = []
        ...
        for miss_day in missing_days: # Iterate through all the missing days
            ...
            if fill_n_days!="all":
                nearest_days = nearest_days[::-1][:fill_n_days] # keep only the specified number of previous days
            if len(nearest_days)==0: # Missing day withous previous days: it's a day to remove
